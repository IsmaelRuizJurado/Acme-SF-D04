...
public class AuthenticatedDeveloperCreateService extends AbstractService<Authenticated, Developer> {
	...
	public void validate(final Developer object) {
		...
		if (!super.getBuffer().getErrors().hasErrors("degree"))
			super.state(this.auxiliarService.validateTextImput(object.getDegree()), "degree", "authenticated.developer.form.error.spam");
		if (!super.getBuffer().getErrors().hasErrors("skills"))
			super.state(this.auxiliarService.validateTextImput(object.getSkills()), "skills", "authenticated.developer.form.error.spam");
		if (!super.getBuffer().getErrors().hasErrors("specialisation"))
			super.state(this.auxiliarService.validateTextImput(object.getSpecialisation()), "specialisation", "authenticated.developer.form.error.spam");
	}
