...
public class ManagerProjectUserStoryCreateService extends AbstractService<Manager, ProjectUserStory> {
	...
	public void validate(final ProjectUserStory object) {
		...
		if (!super.getBuffer().getErrors().hasErrors("userStory") && !super.getBuffer().getErrors().hasErrors("project")) {
			final Collection<UserStory> userstories = this.repository.findUserStoriesByProject(object.getProject().getId());
			super.state(userstories.isEmpty() || !userstories.contains(object.getUserStory()), "project", "manager.projectUserStory.form.error.userstory");
		}
	...
	public void unbind(final ProjectUserStory object) {
		...
		projects = this.repository.findNonPublishedProjectsByManager(manager);
